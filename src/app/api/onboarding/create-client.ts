  ...
  try {
    ...
    const { data: clientData, error: clientError } = await supabase
      ...
      .insert({
        ...
        slug: `${companyName.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`, // Unique slug
        name: companyName,
        trial_status: 'active',
    ...
    const { data: expoData, error: expoError } = await supabase
      .from('expos') // Assuming expos table exists
      .insert({
        id: crypto.randomUUID().toString(),
        client_id: clientId,
        ...
        status: 'active',
        trial_expo: true, // Mark as trial expo
        metadata: {
          created_at: new Date().toISOString(),
          trial: true,
        },
    ...
    // Note: expoError is ok - expos table might not exist yet
    const expoId = expoData?.id || null;
