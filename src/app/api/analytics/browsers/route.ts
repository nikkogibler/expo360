...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📊 Fetching browsers data from Google Analytics...');

    const result = await googleAnalyticsService.getVisitorsByBrowser({
      since,
      until,
      limit: 10
    });
    ...
    console.log('✅ Browsers data fetched successfully:', result.data.length, 'results');
