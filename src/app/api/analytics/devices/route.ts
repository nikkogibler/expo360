...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('📊 Fetching devices data from Google Analytics...');

    const result = await googleAnalyticsService.getVisitorsByDevice({
      since,
      until,
      limit: 10
    });
    ...
    console.log('✅ Devices data fetched successfully:', result.data.length, 'results');
