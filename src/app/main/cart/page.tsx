      ...
      <div className="flex flex-col items-center text-xs text-gray-600 mt-1 w-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 pr-1">
        {parts.map((part, i) => {
          ...
          return (
            ...
            <div key={i} className="flex items-center mb-0.5 last:mb-0 w-full justify-center shrink-0 whitespace-nowrap"> 
              {colorHex ? (
                <span
                  className="w-3 h-3 rounded-full mr-1 border border-gray-300 shrink-0"
                  style={{ backgroundColor: colorHex }}
                ></span>
              ) : isTransparent ? ( 
                <span
                  className="w-3 h-3 rounded-full mr-1 border border-gray-300 shrink-0 bg-white" 
                ></span>
              ) : null}
  ...
  return (
    <Link href={productDetailPath} passHref>
      ...
      >
        ...
        <h3 className="text-sm font-semibold text-gray-800 text-center mt-auto shrink-0 flex items-center justify-center px-1"
        ...
        {item.quantity > 1 && (
          <p className="text-xs font-bold text-gray-700 mt-1 shrink-0">Cantidad: {item.quantity}</p>
        )}
        <div className="shrink-0" style={{ minHeight: '50px', maxHeight: '70px', flexGrow: 1 }}>
...
export default function KusamCartPage() {
  ...
  const [eventLogoUrl, setEventLogoUrl] = useState<string>('/expo1.png');
